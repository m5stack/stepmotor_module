...
void serial_write(uint8_t data) {
 ...
 // UCSR0B |=  (1 << UDRIE0); //modify by M5STack. 
}
...
void requestEvent()  //I2C request data from core. modify by M5STack. 
{
   if(tx_buffer_head ==tx_buffer_tail) return;
 //if(sys.state==STATE_IDLE) Wire.write('I');else Wire.write('B');
  uint8_t tail = tx_buffer_tail;
  Wire.write(tx_buffer[tail]);
    // Update tail position
  tail++;
  if (tail == TX_BUFFER_SIZE) { tail = 0; }
    tx_buffer_tail = tail;
